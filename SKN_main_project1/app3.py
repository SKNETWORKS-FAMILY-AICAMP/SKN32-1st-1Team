...
st.markdown(
    ...
    section[data-testid="stSidebar"] {
        ...
        --primary-color: var(--blue-600) !important;
        accent-color: var(--blue-600) !important;
    }

    /* 토글/세그먼트 컨트롤 클릭 시 깜빡이는 커서 제거 */
    ...
    div[data-baseweb="select"] > div:focus-within {
        border-color: var(--blue-600) !important;
        box-shadow: 0 0 0 2px rgba(36, 119, 189, 0.12) !important;
    }
    ...
    ul[role="listbox"] {
        ...
        box-shadow: 0 12px 28px rgba(20, 76, 123, 0.14) !important;
    }
...
def style_chart(fig):
    fig.update_layout(
        ...
        yaxis=dict(tickformat=',d') # 세로축 쉼표 및 스케일 자동 조절 공통 적용
    )
...
def model_2_region_analysis():
    ...
    if analysis_type == "연료별 조회":
        ...
        if not df_display.empty:
            ...
            display_cols = ['기준년월', '연료', '차종', '이용목적', '등록대수']
    ...
    if view_mode == "그래프":
        if filtered_df.empty:
            ...
            st.markdown(
                ...
                .st-key-gender_age_chart_card [data-testid="stPlotlyChart"] {
                    background-color: #ffffff;
                }
                </style>
                """,
...
# [모델 4] 연간 자동차 등록 추이 (테이블 연도 콤마 현상 수정 완료)
def model_4_yearly_stats():
    ...
    # 2007 포맷 현상 수정 로직 적용 ("년" 붙인 문자열로 변환)
    ...
    if view_mode == "그래프":
        if filtered_df.empty:
            ...
            st.markdown(
                ...
                .st-key-yearly_chart_card [data-testid="stPlotlyChart"] {
                    background-color: #ffffff;
                }
                </style>
                """,
        ...
        # 💡 [핵심 수정] 숫자형 원본 '연도' 컬럼을 삭제하고 '2007년' 문자형인 'year_disp'를 '연도'로 덮어쓰기
    ...
    if view_mode == "그래프":
        if df_merge.empty:
            ...
            st.markdown(
                ...
                .st-key-region_rank_chart_card [data-testid="stPlotlyChart"] {
                    background-color: #ffffff;
                }
                </style>
                """,
...
if model_choice == "1. 연식 차량 대수 조회":
    model_1_age_stats()
elif model_choice == "2. 연료/배기량별 차량등록 수":
    model_2_region_analysis()
elif model_choice == "3. 성별·연령별 등록 추이":
    model_3_gender_age_stats()
elif model_choice == "4. 연간 자동차 등록 추이":
    model_4_yearly_stats()
elif model_choice == "5. 지역별 등록 증감 순위":
    model_5_region_ranking()